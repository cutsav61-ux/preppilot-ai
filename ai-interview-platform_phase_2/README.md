...
**Phase 2 — Dashboard.** A premium, real-data dashboard home is live: profile summary, stats (from `User.stats`), streak widget, quick actions, and a chart-ready score trend panel. Interview generation, history, and settings are intentionally still empty — those routes are visibly marked "Soon" in the nav rather than 404ing. The AI interview engine and analytics land in later phases.
