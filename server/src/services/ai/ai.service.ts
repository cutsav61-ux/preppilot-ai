...
const evaluationResponseSchema = z.object({
  questionEvaluations: z.array(
    z.object({
      ...
      keyConceptsMissed: z.array(z.string()).default([]),
      difficultyEstimate: z.enum(["easy", "medium", "hard"]).default("medium"),
  ...
  overallFeedback: z.object({
    ...
    studyPlan: z.array(z.string()).default([]),
    hiringRecommendation: z.string().min(1),
    companyReadiness: z.string().default("Not enough data to assess yet."),
  }),
});
...
/**
 * Picks which provider to try first. AI_PROVIDER is the tie-breaker, but a
 * provider whose API key isn't configured is never chosen over one that
 * is — e.g. if only GEMINI_API_KEY is set, Gemini is used as primary
 * regardless of AI_PROVIDER's default, instead of wasting a call on a
 * provider that's guaranteed to fail.
 */
function resolveProviderOrder() {
  const anthropicConfigured = Boolean(env.ANTHROPIC_API_KEY);
  const geminiConfigured = Boolean(env.GEMINI_API_KEY);
  const prefersGemini = env.AI_PROVIDER === "gemini";

  let primaryIsGemini: boolean;
  if (geminiConfigured && !anthropicConfigured) {
    primaryIsGemini = true;
  } else if (anthropicConfigured && !geminiConfigured) {
    primaryIsGemini = false;
  } else {
    primaryIsGemini = prefersGemini;
  }

  return {
    primary: primaryIsGemini ? geminiProvider : anthropicProvider,
    primaryName: primaryIsGemini ? "gemini" : "anthropic",
    secondary: primaryIsGemini ? anthropicProvider : geminiProvider,
    secondaryName: primaryIsGemini ? "anthropic" : "gemini",
    secondaryConfigured: primaryIsGemini ? anthropicConfigured : geminiConfigured,
  };
}

async function callProvider(params: GenerateQuestionsParams): Promise<string> {
  const { primary, primaryName, secondary, secondaryName, secondaryConfigured } =
    resolveProviderOrder();

  try {
    return await primary.generateQuestionsRaw(params);
  } catch (error) {
    if (!secondaryConfigured) throw error;
    logger.warn(
      { err: error, provider: primaryName, fallingBackTo: secondaryName },
      "Primary AI provider failed for question generation — trying the secondary provider",
    );
    return secondary.generateQuestionsRaw(params);
  }
}
...
async function callEvaluationProvider(params: EvaluateInterviewParams): Promise<string> {
  ...
  try {
    return await primary.generateEvaluationRaw(params);
  } catch (error) {
    if (!secondaryConfigured) throw error;
    logger.warn(
      { err: error, provider: primaryName, fallingBackTo: secondaryName },
      "Primary AI provider failed for evaluation — trying the secondary provider",
    );
    return secondary.generateEvaluationRaw(params);
  }
}
...
export const aiService = {
  async generateQuestions(params: GenerateQuestionsParams): Promise<GeneratedQuestion[]> {
    try {
      ...
      logger.warn(
        { err: error, attemptedProvider: resolveProviderOrder().primaryName },
    ...
    try {
      ...
      const questionEvaluations = params.questions.map((q) => {
        ...
        return {
          ...
          mistakes: [],
          keyConceptsMissed: [],
          difficultyEstimate: "medium" as const,
          followUpQuestions: [],
        };
      });
