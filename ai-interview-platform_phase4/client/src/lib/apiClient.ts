...
async function rawRequestEnvelope<T>(path: string, options: RequestOptions = {}): Promise<ApiEnvelope<T>> {
...
async function rawRequest<T>(path: string, options: RequestOptions = {}): Promise<T> {
  const envelope = await rawRequestEnvelope<T>(path, options);
...
/**
 * Same as `request`, but resolves the full envelope (including `meta`)
 * instead of just `data` — used for paginated list endpoints.
 */
async function requestEnvelope<T>(path: string, options: RequestOptions = {}): Promise<ApiEnvelope<T>> {
  const accessToken = useAuthStore.getState().accessToken ?? undefined;

  try {
    return await rawRequestEnvelope<T>(path, { ...options, accessToken });
  } catch (error) {
    const isAuthEndpoint = path.startsWith("/auth/");
    const isUnauthorized = error instanceof ApiError && error.status === 401;
    ...
    if (isUnauthorized && !isAuthEndpoint && !options._isRetry) {
      try {
        const { user, accessToken: newAccessToken } = await refreshSession();
        useAuthStore.getState().setSession(user, newAccessToken);
        return await rawRequestEnvelope<T>(path, {
          ...options,
          accessToken: newAccessToken,
          _isRetry: true,
        });
...
export const apiClient = {
  ...
  /** For endpoints that return pagination `meta` alongside `data` (e.g. GET /interviews). */
  getPaginated: async <T>(path: string, options?: RequestOptions) => {
